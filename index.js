...
const strikeMap = {}; // Simpan strike user

client.on('qr', (qr) => {
  console.log('Scan QR ini pake WhatsApp kamu:');
...
client.on('message', async (msg) => {
  ...
  if (isToxic) {
    ...
    if (!strikeMap[userId]) strikeMap[userId] = 0;
    strikeMap[userId] += 1;
    ...
    if (strikeMap[userId] >= 3) {
      ...
      await chat.sendMessage(`@${sender.id.user} jangan toxic ya! Strike ${strikeMap[userId]}/3`, {
        mentions: [sender]
      });
    }
  }
